...
$id = $_POST["sp_ID"]; // 獲取使用者輸入的 sp_ID 值
$subject = $_POST["sp_Subject"]; // 獲取使用者輸入的 sp_Subject 值
$answer = $_POST["sp_Answer"]; // 獲取使用者輸入的 sp_Answer 值

// 將 Blob 和文字資料一起插入 MySQL
$sql = 'INSERT INTO calculus (sp_Photo, sp_ID, sp_Subject, sp_Answer) VALUES (?, ?, ?, ?)';
...
<script>
  // 當使用者選擇圖片時觸發事件
  document.getElementById("sp_Photo").addEventListener("change", function() {
