...
if(isset($_POST['register']))
{
  ...
  if($result)
  {
    if(mysqli_num_rows($result) > 0)    #it will be executed if username or email is already taken
    {
      ...
      {
        #error  for email already reigestered
        echo "
        <script>
    ...
    else #it will be executed if no one has taken username or email before
    {
      ...
      $query = "INSERT INTO `registered_users`(`full_name`, `username`, `email`, `password`) VALUES ('$_POST[cna]','$_POST[username]','$_POST[email]','$password')";
