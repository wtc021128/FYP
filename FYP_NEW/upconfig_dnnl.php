...
$id = $_POST["dnnl_ID"]; // 獲取使用者輸入的 dnnl_ID 值
$subject = $_POST["dnnl_Subject"]; // 獲取使用者輸入的 dnnl_Subject 值
$answer = $_POST["dnnl_Answer"]; // 獲取使用者輸入的 dnnl_Answer 值

// 將 Blob 和文字資料一起插入 MySQL
$sql = 'INSERT INTO calculus (dnnl_Photo, dnnl_ID, dnnl_Subject, dnnl_Answer) VALUES (?, ?, ?, ?)';
...
<script>
  // 當使用者選擇圖片時觸發事件
  document.getElementById("dnnl_Photo").addEventListener("change", function() {
