...
$id = $_POST["dg_ID"]; // 獲取使用者輸入的 dg_ID 值
$subject = $_POST["dg_Subject"]; // 獲取使用者輸入的 dg_Subject 值
$answer = $_POST["dg_Answer"]; // 獲取使用者輸入的 dg_Answer 值

// 將 Blob 和文字資料一起插入 MySQL
$sql = 'INSERT INTO calculus (dg_Photo, dg_ID, dg_Subject, dg_Answer) VALUES (?, ?, ?, ?)';
...
<script>
  // 當使用者選擇圖片時觸發事件
  document.getElementById("dg_Photo").addEventListener("change", function() {
