...
$id = $_POST["q_ID"]; // 獲取使用者輸入的 q_ID 值
$subject = $_POST["q_Subject"]; // 獲取使用者輸入的 q_Subject 值
$answer = $_POST["q_Answer"]; // 獲取使用者輸入的 q_Answer 值

// 將 Blob 和文字資料一起插入 MySQL
$sql = 'INSERT INTO calculus (q_Photo, q_ID, q_Subject, q_Answer) VALUES (?, ?, ?, ?)';
...
<script>
  // 當使用者選擇圖片時觸發事件
  document.getElementById("q_Photo").addEventListener("change", function() {
